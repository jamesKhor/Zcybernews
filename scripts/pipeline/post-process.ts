...
export function postProcessArticle(
  ...
  // Live generation is strict: every IOC must be rediscovered from the body
  // AND source corpus. LLM-only domains/URLs/emails/file paths/registry keys
  // are stripped instead of trusted.
  article.iocs = extractIocs({
    body: article.body,
    sourceText,
    existing: article.iocs,
    includeDomains: true,
