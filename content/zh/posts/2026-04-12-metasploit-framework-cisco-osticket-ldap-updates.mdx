...
## 检测与溯源查询
防御者应该专注于检测与这些工具相关的行为，而不是工具本身。对于新的漏洞利用模块，监控 Web 应用程序日志（Cisco SD-WAN Manager、osTicket）以寻找异常请求模式、登录失败的突发或导致 shell 产生的利用尝试。对于 LDAP/ADCS 增强功能，寻找来自单个主机的异常 LDAP 查询量，特别是那些枚举证书模板或用户 SPN 的查询。对于 Windows 服务持久性的检测，应寻找具有不寻常二进制路径或用户上下文的新服务的创建，特别是配置为自动启动的服务。一个用于服务创建的 Sigma 规则示例可能包括 `EventID: 4697`（服务已安装）和 `ServiceAccount` 字段，该字段是用户主体名称而不是本地或内置帐户。
