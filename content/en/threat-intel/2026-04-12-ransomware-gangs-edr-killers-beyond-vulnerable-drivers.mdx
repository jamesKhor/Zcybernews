...
Detection should focus on kernel-level activity and driver loading events, particularly for new or rarely seen drivers. Hunting queries should examine:
- System logs for the loading of drivers with unusual metadata (e.g., recent timestamps, publisher names mismatching common vendors) via events like `Sysmon Event ID 6` (Driver loaded).
- Process termination events (`Sysmon Event ID 5`) where the parent process is a kernel-mode driver or a system utility like `svchost.exe` or `services.exe` acting anomalously.
- Command-line logging for the use of `PnPUtil.exe` or `sc.exe` to install drivers outside of standard software deployment or update cycles.
- Monitoring for the creation of unsigned or suspiciously signed kernel-mode drivers in the `C:\Windows\System32\drivers\` directory.
