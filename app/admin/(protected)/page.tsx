...
import { listReviewQueueRuns } from "@/lib/editorial-review-admin";
...
export default async function AdminDashboard() {
  ...
  const queueRuns = listReviewQueueRuns();
  const latestQueueRun = queueRuns[0];
  const pendingReviewCount = queueRuns.reduce(
    (total, run) => total + run.statusCounts.pending,
    0,
  );
    ...
    <div className="p-6 space-y-6">
      ...
      <div>
        ...
        <div className="grid grid-cols-1 gap-3 lg:grid-cols-3">
          <Link
            href="/admin/review"
            className="flex items-center gap-3 p-4 rounded-lg bg-gray-900 border border-gray-800 hover:border-amber-700 hover:bg-gray-800 transition-all group"
          >
            <div className="w-9 h-9 rounded-md bg-amber-950 flex items-center justify-center">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">
                {latestQueueRun
                  ? `${latestQueueRun.runKey} · ${pendingReviewCount} pending`
                  : "No queued candidates yet"}
              </p>
            </div>
          </Link>
          <Link
      ...
      <div>
        ...
        <div className="space-y-2">
          ...
          {recentPosts.map((post) => {
            const isTi = enTi.some(
              (t) => t.frontmatter.slug === post.frontmatter.slug,
            );
            const zhExists = zhPosts.some(
              (z) => z.frontmatter.slug === post.frontmatter.slug,
            );
            return (
              <div
              ...
              >
                ...
                <Link
                  href={`/en/${isTi ? "threat-intel" : "articles"}/${post.frontmatter.slug}`}
                  target="_blank"
        ...
        <div className="space-y-2">
          {sources.map((source) => (
            ...
            >
              ...
              <span
                className={`text-xs px-2 py-0.5 rounded-full ${source.enabled ? "bg-emerald-950 text-emerald-400" : "bg-gray-800 text-gray-500"}`}
              >
